...
export default function Products({ initialProduct = 'dmk', singleMode = false }) {
  ...
  const navVisible = useScrollAnimation(navRef, { observeKey: active });
  const featureBlocksVisible = useScrollAnimation(featureBlocksRef, { threshold: 0.08, observeKey: active });
  const hmiVisible = useScrollAnimation(hmiSectionRef, { threshold: 0.08, observeKey: active });
  const demoEmbedVisible = useScrollAnimation(demoEmbedRef, { threshold: 0.08, observeKey: active });
  const amcBenefitsVisible = useScrollAnimation(amcBenefitsRef, { threshold: 0.08, observeKey: active });
  const amcGameChangerVisible = useScrollAnimation(amcGameChangerRef, { threshold: 0.08, observeKey: active });
  const amcSensorlessVisible = useScrollAnimation(amcSensorlessRef, { threshold: 0.08, observeKey: active });
  const comparisonVisible = useScrollAnimation(comparisonRef, { threshold: 0.08, observeKey: active });
  const lciProblemVisible = useScrollAnimation(lciProblemRef, { threshold: 0.1, observeKey: active });
  const lciShiftVisible = useScrollAnimation(lciShiftRef, { threshold: 0.1, observeKey: active });
  const lciWhyVisible = useScrollAnimation(lciWhyRef, { threshold: 0.08, observeKey: active });
  const lciCtaVisible = useScrollAnimation(lciCtaRef, { threshold: 0.1, observeKey: active });
